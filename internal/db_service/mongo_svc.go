...
func NewMongoService[DocType interface{}](config MongoServiceConfig, collection string) DbService[DocType] {
	...
	if svc.ServerHost == "" {
		svc.ServerHost = enviro("RESERVER_API_MONGODB_HOST", "localhost")
	}

	if svc.ServerPort == 0 {
		port := enviro("RESERVER_API_MONGODB_PORT", "27017")
	...
	if svc.UserName == "" {
		svc.UserName = enviro("RESERVER_API_MONGODB_USERNAME", "")
	}

	if svc.Password == "" {
		svc.Password = enviro("RESERVER_API_MONGODB_PASSWORD", "")
	}

	if svc.DbName == "" {
		svc.DbName = enviro("RESERVER_API_MONGODB_DATABASE", "reserver")
	}

	if svc.Timeout == 0 {
		seconds := enviro("RESERVER_API_MONGODB_TIMEOUT_SECONDS", "10")
